...
(function(){
  if (window.__schoolhubMobileOverviewNoCodeFoldIdlineFix) return;
  window.__schoolhubMobileOverviewNoCodeFoldIdlineFix = true;
  ...
  function buildOverviewCardsNoFoldCode(){
    ...
    box.innerHTML = rows.map(function(row, idx){
      ...
      var bonusColIdx3=-1,starColIdx3=-1,totalIdx3=cells.length-2,gradeIdx3=cells.length-1;
      for(var ci=0;ci<headers.length;ci++){
        var hcls=headers[ci].className||'';
        if(hcls.indexOf('summary-total-col')>=0) totalIdx3=ci;
        else if(hcls.indexOf('summary-grade-col')>=0) gradeIdx3=ci;
        else if(hcls.indexOf('sh-bonus-col')>=0) bonusColIdx3=ci;
        else if(hcls.indexOf('sh-star-col')>=0) starColIdx3=ci;
      }
      var total = cells[totalIdx3] ? cleanText(cells[totalIdx3]) : '0';
      var grade = cells[gradeIdx3] ? cleanText(cells[gradeIdx3]) : '-';
      var bonus3 = bonusColIdx3>=0&&cells[bonusColIdx3] ? cleanText(cells[bonusColIdx3]) : '-';
      var stars3 = starColIdx3>=0&&cells[starColIdx3] ? cleanText(cells[starColIdx3]).replace(/[^\d]/g,'') : '-'; if(stars3===''||stars3==='0') stars3='-';
      var planHtml = '';
      for (var i = attIndex + 1; i < totalIdx3; i++) {
        if(i===bonusColIdx3||i===starColIdx3) continue;
      ...
      var bonusStarHtml3='<div class="summary-mobile-bonus-star"><div class="summary-mobile-bonus-cell" onclick="if(window.shOvShowBonusDetail)window.shOvShowBonusDetail(this)"><span>+โบนัส</span><b>'+esc(bonus3)+'</b></div><div class="summary-mobile-star-cell"><span>⭐ดาว</span><b>'+esc(stars3)+'</b></div></div>';
  ...
  window.buildMobileOverviewCards = buildOverviewCardsNoFoldCode;
  function hookRender(){
    if (typeof window.renderCourseOverview === 'function' && !window.renderCourseOverview.__mobileNoFoldCodeIdlineWrapped) {
      var old = window.renderCourseOverview;
      var fn = function(){
        var r = old.apply(this, arguments);
        setTimeout(buildOverviewCardsNoFoldCode, 50);
        setTimeout(buildOverviewCardsNoFoldCode, 200);
        return r;
      };
      fn.__mobileNoFoldCodeIdlineWrapped = true;
      window.renderCourseOverview = fn;
    }
  }
  hookRender();
  document.addEventListener('DOMContentLoaded', function(){ hookRender(); setTimeout(buildOverviewCardsNoFoldCode, 300); });
  window.addEventListener('resize', function(){ setTimeout(buildOverviewCardsNoFoldCode, 100); });
  document.addEventListener('click', function(e){ if(window.isDropdownRelatedElement && window.isDropdownRelatedElement(e.target)) return; setTimeout(buildOverviewCardsNoFoldCode, 140); });
})();
